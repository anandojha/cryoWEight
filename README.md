...
A single `config.xml` specifies each system and holds the MD, weighted ensemble, imaging, target selection, and convergence parameters. A new system starts from the `config.xml` of an example and modifies it. The number of outer rounds is given on the command line, `iterate --range 2 N` for a fixed count or `iterate --until-converged` for the KL stopping rule.
