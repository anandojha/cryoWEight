...
Platform precedence: the WEST_PLATFORM environment variable (set by env.sh /
run.sh, default CPU) overrides seg_config.json["platform"]. This is how
`run.sh ... --platform CPU|CUDA` reaches the per-segment OpenMM context
without rewriting the committed seg_config.json.
"""
import os
import json
...
cfg["platform"] = os.environ.get("WEST_PLATFORM", cfg["platform"])
